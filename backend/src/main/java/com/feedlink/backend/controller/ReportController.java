...
public class ReportController {
    ...
    public ResponseEntity<?> downloadPredictionsReport(
            @RequestParam(defaultValue = "pdf") String format,
            @AuthenticationPrincipal User user
    ) throws IOException {
        List<AIPredictionLog> logs = aiPredictionRepository.findAll();
        
        if ("csv".equalsIgnoreCase(format)) {
            StringBuilder csv = new StringBuilder();
            csv.append("FeedLink AI Prediction History Report\n");
            csv.append("Generated By:,").append(user.getName()).append(" (").append(user.getEmail()).append(")\n");
            csv.append("Date:,").append(java.time.LocalDate.now().toString()).append("\n\n");
            
            csv.append("ID,Food Type,Category,Confidence,Freshness Score,Estimated Servings,Uploader,Timestamp,Explanation\n");
            for (AIPredictionLog log : logs) {
                csv.append(log.getId()).append(",")
                   .append(escapeCsv(log.getFoodType())).append(",")
                   .append(escapeCsv(log.getCategory())).append(",")
                   .append(String.format("%.4f", log.getConfidence())).append(",")
                   .append(log.getFreshnessScore()).append(",")
                   .append(log.getEstimatedServings() != null ? log.getEstimatedServings() : 15).append(",")
                   .append(escapeCsv(log.getUserEmail())).append(",")
                   .append(log.getTimestamp() != null ? log.getTimestamp().toString() : "").append(",")
                   .append(escapeCsv(log.getPredictionRaw())).append("\n");
            }
            return ResponseEntity.ok()
                    .header(HttpHeaders.CONTENT_DISPOSITION, "attachment; filename=feedlink_predictions_report.csv")
                    .contentType(MediaType.parseMediaType("text/csv"))
                    .body(csv.toString());
        } else {
            ByteArrayOutputStream out = new ByteArrayOutputStream();
            ...
            try {
                ...
                Font titleFont = FontFactory.getFont(FontFactory.HELVETICA_BOLD, 18);
                Paragraph title = new Paragraph("FeedLink AI Prediction Logs Audit Report", titleFont);
                title.setAlignment(Element.ALIGN_CENTER);
                title.setSpacingAfter(20);
                document.add(title);
                
                Paragraph info = new Paragraph("Generated By: " + user.getName() + " (" + user.getEmail() + ")\n" +
                        "Date: " + java.time.LocalDate.now().toString() + "\n\n");
                document.add(info);
                ...
                table.addCell("ID");
                table.addCell("Food Type");
                table.addCell("Category");
                table.addCell("Confidence");
                table.addCell("Freshness");
                table.addCell("Uploader");
                table.addCell("Timestamp");
                
                for (AIPredictionLog log : logs) {
                    table.addCell(log.getId() != null ? (log.getId().length() > 8 ? log.getId().substring(0, 8) : log.getId()) : "N/A");
                    table.addCell(log.getFoodType());
                    table.addCell(log.getCategory());
                    table.addCell(String.format("%.1f%%", log.getConfidence() * 100.0));
                    table.addCell(String.format("%.1f%%", log.getFreshnessScore()));
                    table.addCell(log.getUserEmail());
                    table.addCell(log.getTimestamp() != null ? log.getTimestamp().format(DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm")) : "N/A");
                }
                document.add(table);
                document.close();
            } catch (Exception e) {
            ...
            byte[] pdfBytes = out.toByteArray();
            return ResponseEntity.ok()
                    .header(HttpHeaders.CONTENT_DISPOSITION, "attachment; filename=feedlink_predictions_report.pdf")
                    .contentType(MediaType.APPLICATION_PDF)
                    .body(pdfBytes);
        }
    }
